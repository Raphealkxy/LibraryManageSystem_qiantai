...
import com.timmy.service.impl.LoginService;
...
public class LoginController {
   ...
   private LoginService loginService;
    ...
    public ModelAndView UserLogin(User user, HttpSession httpSession,String checkcode) throws Exception {
        if(checkcode!=""&&checkcode!=null){
            ...
            if(!code.equals(checkcode)){
                ...
                modelAndView.setViewName("qiantai/myaccount");
                return  modelAndView;
            }
        }else {
            ModelAndView modelAndView=new ModelAndView();
            modelAndView.addObject("checkcodeError","*填写验证码");
            modelAndView.addObject("user",user);
            modelAndView.setViewName("qiantai/myaccount");
            return  modelAndView;
        ...
        List<ItemsUser> list=loginService.UserLogin(username);
        if(list.size()>0){
            ItemsUser itemsUser=list.get(0);
            if(itemsUser.getState()==0){
                //用户没有激活
                ModelAndView modelAndView=new ModelAndView();
                modelAndView.addObject("usernameError","*用户没有激活");
                modelAndView.addObject("checkcodeError","");
                modelAndView.addObject("user",user);
                modelAndView.setViewName("qiantai/myaccount");
                return  modelAndView;
            } else if(itemsUser.getPassword().equals(user.getPassword())){
                //登陆成功
               //将用户的信息放入到session中去。
                httpSession.setAttribute("user",itemsUser);
    ...
    public String getCheckImg(HttpSession httpSession,HttpServletResponse response) throws IOException {
        int width = 120;
        int height = 30;
